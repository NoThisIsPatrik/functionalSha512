...
v80 = lambda h,v,i,w : (v80(h,(lambda T:((T+((v[0]>>28)^(v[0]>>34)^(v[0]>>39)^(v[0]<<25)^(v[0]<<30)^(v[0]<<36))+((v[0]&v[1])^(v[0]&v[2])^(v[1]&v[2])))&((1<<64)-1),v[0],v[1],v[2],(v[3]+T)&((1<<64)-1),v[4],v[5],v[6]))(v[7]+((v[4]>>14)^(v[4]<<50)^(v[4]>>18)^(v[4]<<46)^(v[4]>>41)^(v[4]<<23))+(v[4]&v[5])+((~v[4])&v[6])+
    ...
    [i]+w[i]),i+1,w)if i<80 else [(x+y)&((1<<64)-1) for x,y in zip(h,v)])

X = lambda w:X(w+[(w[-16]+w[-7]+((w[-15]>>1)^(w[-15]<<63)^(w[-15]>>8)^(w[-15]<<56)^(w[-15]>>7))+((w[-2]>>19)^(w[-2]<<45)^(w[-2]>>61)^(w[-2]<<3)^(w[-2]>>6)))&((1<<64)-1)])if len(w)<80 else w

P = lambda m,h : P(m[128:],v80(h,h,0,X(list(struct.unpack('!16Q',m[:128])))))if m else struct.pack("!8Q",*h)

sha512 = lambda m : (P(m+b"\x80"+b"\x00"*((128-16-1)+(128*((len(m)%128)>(128-16-1)))-(len(m)%128))+(b"\x00"*8)+struct.pack("!Q",len(m)*8),(0x6a09e667f3bcc908,0xbb67ae8584caa73b,0x3c6ef372fe94f82b,0xa54ff53a5f1d36f1,0x510e527fade682d1,0x9b05688c2b3e6c1f,0x1f83d9abfb41bd6b,0x5be0cd19137e2179)))
