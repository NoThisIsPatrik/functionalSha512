...
__license__ = 'LGPL3.0'
__description__ = "A sha512 implementation with only pure functions and immutable values"
...
expand_state_w = (lambda w : expand_state_w( 
        w + [ mask( w[-16] + f0(w) + w[-7] + f1(w) ) ]) if len(w)<80 else w )
...
v80_h = lambda _h,v,i,w : ( v80_h( _h, (mask(t1(v,i,w)+t2(v)), v[0], v[1], v[2], mask(v[3] + t1(v,i,w)), v[4], v[5], v[6]), i+1, w) if i<80 else vsum(_h,v) )

vsum = lambda a,b: [mask(x+y) for x,y in zip(a, b)]

do_chunk = lambda c,_h: v80_h( _h, _h, 0, expand_state(c) ) 

pad = lambda m:( m + b"\x80" + b"\x00"*((128-16-1)+(128*((len(m)%128)>(128-16-1)))-(len(m)%128)) + (b"\x00"*8) + struct.pack("!Q",len(m)*8) )

pr = lambda m, h: ( pr( m[128:], do_chunk(m[:128], h)) if m else struct.pack("!8Q",*h) )
sha512 = lambda m : (pr( pad(m), ( 0x6a09e667f3bcc908, 0xbb67ae8584caa73b, 0x3c6ef372fe94f82b, 0xa54ff53a5f1d36f1, 0x510e527fade682d1, 0x9b05688c2b3e6c1f, 0x1f83d9abfb41bd6b, 0x5be0cd19137e2179 ) ))
